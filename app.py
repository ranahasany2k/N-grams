...
def print_corpus_sample(file_path, num_sentences=2):
    with open(file_path, "r") as file:
        corpus = file.readlines()
        st.write("Sample from the corpus:")
        for i in range(num_sentences):
            st.write(corpus[i])


def generate_unigram_sentences(unigram_smoothed):
    unigram_smoothed.generateSentencesToFile(5, "smoothed_unigram_output.txt")
    with open("smoothed_unigram_output.txt", "r") as file:
        for line in file:
            words = line.split()
            line_to_print = ' '.join(words)
            st.write(line_to_print, end='', flush=True)
            time.sleep(0.1)
            st.write()

def generate_bigram_sentences(bigram_s):
    bigram_s.generateSentencesToFile(5, "bigram_smoothed_output.txt")
    with open("bigram_smoothed_output.txt", "r") as file:
        for line in file:
            words = line.split()
            line_to_print = ' '.join(words)
            st.write(line_to_print, end='', flush=True)
...
if __name__ == "__main__":
    ...
    # Load positive and negative test corpora
    ...
    unigram_dist_smooth = UnigramDist_smoothed(trainCorpus)
    unigram_smoothed = SmoothedUnigramModel(trainCorpus)

    # Instantiate Bigram Model
    bigram_dist_smoothed = BigramDist_smoothed(trainCorpus)
    bigram_s = BigramModel_smoothed(trainCorpus)


    st.title("NLP Model Generator")

    # Add a checkbox to check the actual corpus sample
    if st.checkbox("Check the actual corpus sample"):
        print_corpus_sample('train.txt')

    # Generate Sentences section in the sidebar
    st.sidebar.title('Generate Sentences')

    # Generate Unigram Sentences
    if st.sidebar.checkbox('Generate Unigram Sentences'):
        st.title("Unigram Sentence Generator")
        generate_unigram_sentences(unigram_smoothed)

    # Generate Bigram Sentences
    if st.sidebar.checkbox('Generate Bigram Sentences'):
        st.title("Bigram Sentence Generator")
        generate_bigram_sentences(bigram_s)
